...
server.get('Test', function (req, res, next) {
    var oldFactory = require('*/cartridge/scripts/factories/product');
    var newFactory = require('*/cartridge/scripts/factories/newProduct');
    var product = newFactory.get({ pid: req.querystring.pid, pview: req.querystring.pview || false });
    res.json({ product: product, oldProduct: oldFactory.get({ pid: req.querystring.pid, pview: req.querystring.pview }) });
    next();
});

server.get('OldProduct', function (req, res, next) {
    var oldFactory = require('*/cartridge/scripts/factories/product');
    res.json({ product: oldFactory.get({ pid: req.querystring.pid, pview: 'tile' }) });
    next();
});

server.get('Product', function (req, res, next) {
    var newFactory = require('*/cartridge/scripts/factories/newProduct');
    res.json({ product: newFactory.get({ pid: req.querystring.pid, pview: 'tile' }) });
    next();
});

server.get('OldGrid', function (req, res, next) {
    var oldFactory = require('*/cartridge/scripts/factories/product');
    var products = [
    ...
    var productModels = products.map(function (product) {
        return oldFactory.get({ pid: product, pview: 'tile' });
    });
