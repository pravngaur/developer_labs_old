...
$(document).ready(function () {
    $('body').on('click', '.show-more-button', function (e) {
        ...
        var $hideMoreElement = $(this).closest('.look-book-layout').find('.hide-more');
        $hideMoreElement.removeClass('d-none');
    });

    $('body').on('click', '.hide-more-button', function (e) {
        e.preventDefault();

        var $set2Element = $(this).closest('.look-book-layout').find('.look-book-set2');
        $set2Element.addClass('hide-set');

        var $showMoreElement = $(this).closest('.look-book-layout').find('.show-more');
        $showMoreElement.removeClass('d-none');

        var $hideMoreElement = $(this).closest('.look-book-layout').find('.hide-more');
        $hideMoreElement.addClass('d-none');
    });
});
