...
describe('ProductVariation - Get product variation with only master product ID', function () {
    ...
    it('should returns master product details and variant attributes', function (done) {
        ...
        request(myGetRequest, function (error, response) {
            ...
            assert.equal(attrColorBlue.url, resourcePath + 'dwvar_25604455M_color=SLABLFB&pid=25604455M&quantity=1', 'Actual color attribute = SLABLFB: url not as expected.');
            ...
            assert.equal(attrColorWhite.url, resourcePath + 'dwvar_25604455M_color=WHITEFB&pid=25604455M&quantity=1', 'Actual color attribute = WHITEFB: url not as expected.');
            ...
            assert.equal(bodyAsJson.product.variationAttributes[1].values[0].url, resourcePath + 'dwvar_25604455M_size=145&pid=25604455M&quantity=1', 'Actual product.variationAttributes[1].values[0].url not as expected.');

            // Verify URL for product.variationAttributes of Size of id = 150
            assert.equal(bodyAsJson.product.variationAttributes[1].values[1].url, resourcePath + 'dwvar_25604455M_size=150&pid=25604455M&quantity=1', 'Actual product.variationAttributes[1].values[1].url not as expected.');

            // Verify URL for product.variationAttributes of Size of id = 155
            assert.equal(bodyAsJson.product.variationAttributes[1].values[2].url, resourcePath + 'dwvar_25604455M_size=155&pid=25604455M&quantity=1', 'Actual product.variationAttributes[1].values[2].url not as expected.');

            // Verify URL for product.variationAttributes of Size of id = 160
            assert.equal(bodyAsJson.product.variationAttributes[1].values[3].url, resourcePath + 'dwvar_25604455M_size=160&pid=25604455M&quantity=1', 'Actual product.variationAttributes[1].values[3].url not as expected.');

            // Verify URL for product.variationAttributes of Size of id = 165
            assert.equal(bodyAsJson.product.variationAttributes[1].values[4].url, resourcePath + 'dwvar_25604455M_size=165&pid=25604455M&quantity=1', 'Actual product.variationAttributes[1].values[4].url not as expected.');

            // Verify URL for product.variationAttributes of Size of id = 170
            assert.equal(bodyAsJson.product.variationAttributes[1].values[5].url, resourcePath + 'dwvar_25604455M_size=170&pid=25604455M&quantity=1', 'Actual product.variationAttributes[1].values[5].url not as expected.');

            // Verify URL for product.variationAttributes of Size of id = 175
            assert.equal(bodyAsJson.product.variationAttributes[1].values[6].url, resourcePath + 'dwvar_25604455M_size=175&pid=25604455M&quantity=1', 'Actual product.variationAttributes[1].values[6].url not as expected.');

            // Verify URL for product.variationAttributes of Size of id = 180
            assert.equal(bodyAsJson.product.variationAttributes[1].values[7].url, resourcePath + 'dwvar_25604455M_size=180&pid=25604455M&quantity=1', 'Actual product.variationAttributes[1].values[7].url not as expected.');

            // Verify URL for product.variationAttributes of Size of id = 185
            assert.equal(bodyAsJson.product.variationAttributes[1].values[8].url, resourcePath + 'dwvar_25604455M_size=185&pid=25604455M&quantity=1', 'Actual product.variationAttributes[1].values[8].url not as expected.');

            // Verify URL for product.variationAttributes of Size of id = 190
            assert.equal(bodyAsJson.product.variationAttributes[1].values[9].url, resourcePath + 'dwvar_25604455M_size=190&pid=25604455M&quantity=1', 'Actual product.variationAttributes[1].values[9].url not as expected.');

            // Verify URL for product.variationAttributes of Size of id = 200
            assert.equal(bodyAsJson.product.variationAttributes[1].values[10].url, resourcePath + 'dwvar_25604455M_size=200&pid=25604455M&quantity=1', 'Actual product.variationAttributes[1].values[10].url not as expected.');

            // Verify URL for product.variationAttributes of Size of id = 220
            assert.equal(bodyAsJson.product.variationAttributes[1].values[11].url, resourcePath + 'dwvar_25604455M_size=220&pid=25604455M&quantity=1', 'Actual product.variationAttributes[1].values[11].url not as expected.');

            // Verify URL for product.variationAttributes of width = A (32/33)
            assert.equal(bodyAsJson.product.variationAttributes[2].values[0].url, resourcePath + 'dwvar_25604455M_width=A&pid=25604455M&quantity=1', 'Actual product.variationAttributes[2].values[0].url not as expected.');

            // Verify URL for product.variationAttributes of width = B (34/35)
            assert.equal(bodyAsJson.product.variationAttributes[2].values[1].url, resourcePath + 'dwvar_25604455M_width=B&pid=25604455M&quantity=1', 'Actual product.variationAttributes[2].values[1].url not as expected.');
